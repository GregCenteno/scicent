...
function normalize(result, categoryKey) {
  ...
  // MEDLINE cataloga los artículos que no están en inglés con el título
  // traducido al inglés entre corchetes (p. ej. "[Towards precision
  // clinical nutrition...]") y guarda el título real, en el idioma
  // original, en `vernacularTitle`. Cuando Europe PMC lo trae, es lo que
  // hay que mostrar; si no, cleanTitle() al menos quita los corchetes
  // sobrantes para no mostrar "[...]" en pantalla.
  const rawTitle =
    result.vernacularTitle && result.vernacularTitle.trim()
      ? result.vernacularTitle
      : result.title;
  const title = cleanTitle(rawTitle);
  const hook = buildHook(abstract, title);
...
function cleanTitle(title) {
  let t = (title || "Sin título").replace(/<[^>]+>/g, "").trim();
  // Respaldo defensivo: si el título completo quedó envuelto en corchetes
  // (convención de MEDLINE para títulos traducidos), se los quitamos.
